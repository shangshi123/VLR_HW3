...
class ViT(nn.Module):
    ...
    def __init__(self, patch_dim, d_model, d_ff, num_heads, num_layers, num_patches, num_classes, device = 'cuda'):
        ...
        self.patch_embedding = nn.Linear(3*self.patch_dim**2, self.d_model) # TODO (Linear Layer that takes as input a patch and outputs a d_model dimensional vector)
    ...
    def patchify(self, images):
        ...
        images = images.reshape(N,C,self.num_patches,-1).transpose(1,2)
    ...
    def forward(self, images):
        ...
        cls_embed = self.cls_token.repeat(N,1,1)
        output = torch.cat((cls_embed, patches_embedded),dim = 1)# TODO (append a CLS token to the beginning of the sequence of patch embeddings)
        
        output = self.positional_encoding(patches_embedded)
        mask = None
        ## torch.ones((self.num_patches, self.num_patches), device=self.device)
