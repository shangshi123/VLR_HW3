...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        embed = torch.arange(S).to(x.device)
        output = x + self.encoding(embed)
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, seq, cond):
        ...
        # Then add a residual connection to the original input, and finally apply normalization. #############################
        out = self.cross_attn(query = seq,key = cond, value = cond)
        out = self.dropout(out) + seq
        out = self.norm(out)
        return out
...
class TransformerDecoder(nn.Module):
    ...
    def get_data_embeddings(self, features, captions):
        ...
        feature_embedding = self.feature_embedding(features).unsqueeze(dim = 1)
